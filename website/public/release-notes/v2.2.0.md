> Multi-user branch note: this branch continues to use its built-in PostgreSQL multi-user architecture and does not enable upstream QwenPaw Hub. QwenPaw Data source is synchronized, but its runtime entry remains disabled until platform user and Agent authorization are integrated.
